...
const Game = {
  sessionId:null, questions:[], currentQ:0, score1:0, score2:0,
  team1:'', team2:'', t1Answered:false, t2Answered:false,
  t1Choice:-1, t2Choice:-1, timerInterval:null, timeLeft:25,
  ...
  async loadCompetitions() {
    try {
      ...
      comps.forEach(c => {
        ...
        o.value = c.id; o.textContent = `${c.name} (${c.question_count} questions)`;
        sel.appendChild(o);
      });
    } catch(e) { App.notify('Impossible de charger les compétitions.', true); }
  },
  ...
  async start() {
    ...
    try {
      App.setLoading('start-game-btn', true);
      const { session, questions } = await API.startGame({ competition_id:compId, team1_name:t1, team2_name:t2 });
      this.sessionId = session.id; this.questions = questions;
      this.team1 = t1; this.team2 = t2; this.score1 = 0; this.score2 = 0; this.currentQ = 0;
      ['game-t1-name','ptag1'].forEach(id => document.getElementById(id).textContent = t1);
      ['game-t2-name','ptag2'].forEach(id => document.getElementById(id).textContent = t2);
      App.showScreen('game-screen');
      this.loadQuestion();
    } catch(e) { App.notify(e.message, true); }
    finally { App.setLoading('start-game-btn', false); }
  },

  loadQuestion() {
    if (this.currentQ >= this.questions.length) { this.end(); return; }
    const q = this.questions[this.currentQ];
    this.t1Answered = false; this.t2Answered = false; this.t1Choice = -1; this.t2Choice = -1;
    document.getElementById('q-counter').textContent = `Q ${this.currentQ+1}/${this.questions.length}`;
    ...
    App.renderQBody('qbody1', q); App.renderQBody('qbody2', q);
    this.renderAnswers('answers-1', q, 1); this.renderAnswers('answers-2', q, 2);
    this.startTimer();
  },

  renderAnswers(containerId, q, team) {
    const c = document.getElementById(containerId); c.innerHTML = '';
    // For game mode only show QCM/TF answers; other types show simplified
    const answers = Array.isArray(q.answers) ? q.answers : [];
    if (['qcm','truefalse'].includes(q.q_type)) {
      answers.forEach((ans, idx) => {
        ...
        if (!ans.img && !ans.text) inner = `Option ${idx+1}`;
        btn.innerHTML = inner;
        btn.onclick = () => this.handleAnswer(team, idx, q);
        c.appendChild(btn);
      });
    } else {
      // For complex types in game mode, show simplified message
      const div = document.createElement('div');
      div.style.cssText = 'text-align:center;padding:20px;opacity:.6;font-size:.9rem';
      div.textContent = '⚠️ Ce type de question n\'est disponible qu\'en mode Examen';
      c.appendChild(div);
      // Auto-mark both as answered/wrong
      setTimeout(() => {
  ...
  handleAnswer(team, idx, q) {
    ...
    document.querySelectorAll(`#answers-${team} .answer-btn`).forEach((b,i) => {
      b.disabled = true; if (i===idx) b.classList.add('selected');
    });
    const otherDone = team===1 ? this.t2Answered : this.t1Answered;
    if (!otherDone) document.getElementById(`overlay-${team}`).classList.add('show');
  ...
  async reveal(q) {
    try {
      const { correct_index, answers } = await API.checkAnswer(this.sessionId, {
        question_id: q.id, choice_index: this.t1Choice
      });
      const t1ok = this.t1Choice === correct_index;
      const t2ok = this.t2Choice === correct_index;
      ['answers-1','answers-2'].forEach((cid, ti) => {
        const choice = ti===0 ? this.t1Choice : this.t2Choice;
        document.querySelectorAll(`#${cid} .answer-btn`).forEach((b,i) => {
      ...
      if (t1ok) this.score1++; if (t2ok) this.score2++;
      document.getElementById('score-t1').textContent = this.score1;
      document.getElementById('score-t2').textContent = this.score2;
      let pct=50; const tot=this.score1+this.score2;
      if (tot>0) { pct=50-(this.score1-this.score2)*7; pct=Math.max(8,Math.min(92,pct)); }
      document.getElementById('rope-flag').style.left = pct+'%';
      const mk=(name,ok,cls)=>`<div class="reveal-team-card ${ok?'winner':''}"><div class="reveal-result-icon">${ok?'✅':'❌'}</div><div class="reveal-team-name ${cls}">${App.esc(name)}</div><div class="reveal-label">${ok?'Bonne réponse !':'Mauvaise'}</div></div>`;
      document.getElementById('reveal-results').innerHTML = mk(this.team1,t1ok,'red')+mk(this.team2,t2ok,'blue');
      const correctAns = (answers||q.answers)[correct_index];
      document.getElementById('reveal-correct-ans').innerHTML = `✅ Bonne réponse : <strong>${App.esc(correctAns?.text||'(image)')}</strong>`;
      const isLast = this.currentQ+1 >= this.questions.length;
      document.getElementById('reveal-next-btn').textContent = isLast ? '🏁 Résultats' : 'Suivant ➜';
      document.getElementById('round-reveal').classList.add('show');
    } catch(e) { App.notify('Erreur vérification réponse.', true); }
  },

  continueRound() {
    document.getElementById('round-reveal').classList.remove('show');
    this.currentQ++; this.loadQuestion();
  },

  startTimer() {
    clearInterval(this.timerInterval); this.timeLeft = 25; this.updateTimer();
    this.timerInterval = setInterval(() => {
      ...
      if (this.timeLeft <= 0) {
        ...
        document.getElementById('overlay-1').classList.remove('show');
        document.getElementById('overlay-2').classList.remove('show');
        document.querySelectorAll('#answers-1 .answer-btn,#answers-2 .answer-btn').forEach(b=>b.disabled=true);
  ...
  async end() {
    clearInterval(this.timerInterval);
    try { await API.finishGame(this.sessionId, {score1:this.score1, score2:this.score2}); } catch(_){}
    ...
    const w=document.getElementById('result-winner'), t=document.getElementById('result-trophy');
    if (this.score1>this.score2)      { w.textContent=this.team1; w.className='winner-name red';  t.textContent='🏆'; this.confetti(); }
    else if (this.score2>this.score1) { w.textContent=this.team2; w.className='winner-name blue'; t.textContent='🏆'; this.confetti(); }
